...
def change_settings(settings, settings_change_string):
    ...
    for opt in filter(len, map(str.strip, settings_change_string.split(';'))):
        ...
        if k in ('train_dir_seed', 'file', 'train_limit', 'inflate_bounding_box', 'scale_prior', 'indices_suppress_radius', 'spread_radii', 'scale_factor', 'scale_suppress_radius', 'selective_bkg', 'image_size'):
